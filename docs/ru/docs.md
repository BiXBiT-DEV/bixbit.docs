...
## Редактирование страниц через Pages CMS

[Pages CMS](https://pagescms.org/) ([app.pagescms.org](https://app.pagescms.org/)) — это сайт, где вы правите текст, а сохранённые файлы уезжают в репозиторий на GitHub. Войти можно тем же аккаунтом **GitHub**, что и для проекта.
...
Примерно **так же** слова в **боковом меню** сайта могут видеть читатели (подписи берутся из поля **`title`** во frontmatter и из заголовков; для группы — из **`index.md`** или из имени папки и правил генерации сайдбара в **`sidebar.ts`** / `vitepress-sidebar`):
...
Символы **▼ / ►** здесь просто означают развёрнутую и свёрнутую группу; точный вид (иконки, свёрнуто по умолчанию или нет) задаёт тема и **`collapseDepth`** в конфигурации сайдбара.
...
Подробнее про процесс рецензирования и защиту ветки **`main`** см. правила вашей команды в настройках репозитория на GitHub.
