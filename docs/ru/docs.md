...
## **Редактирование страниц через Decap CMS**

В проекте подключена админка Decap CMS. Через неё можно редактировать существующие страницы и создавать новые без ручного редактирования файлов в репозитории.

Админка находится по адресу:

- локально: `http://localhost:5173/bixbit.docs/admin/`
- на опубликованном сайте: `/bixbit.docs/admin/`

По текущей конфигурации Decap работает с GitHub-репозиторием и использует `editorial_workflow`. Это значит, что изменения проходят через черновик и публикацию, а не сразу попадают в итоговый контент.

### Как отредактировать существующую страницу

1. Открой админку и авторизуйся через GitHub.
2. Выбери коллекцию `Documentation Pages`.
3. Найди нужную страницу в списке.
4. Измени поле `Title` или содержимое в поле `Body`.
5. Сохрани изменения как draft или отправь на публикацию, в зависимости от текущего статуса.

### Как добавить новую страницу

1. Открой `Documentation Pages`.
2. Нажми `New Documentation Page`.
3. Заполни `Title`.
4. В поле `Path` укажи путь страницы внутри `docs`.
5. В поле `Body` добавь содержимое страницы на Markdown.
6. Сохрани draft или отправь страницу на публикацию.
...
Через текущую настройку админки редактируются обычные Markdown-страницы из папки `docs`. Кастомные Vue-компоненты, тема VitePress, sidebar-конфиг и файлы в `.vitepress` через эту форму не редактируются.
